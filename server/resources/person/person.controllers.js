...
const getWithCerts = (req, res) => {
  ...
  const query = "match (p1:Person) OPTIONAL MATCH (p1)-[r1:HAS_CERTIFICATION]->(c1:Certification) OPTIONAL MATCH (p1)-[r2:HAS_CERTIFICATION]->(c2:Certification)-[r3:SIGNS_CERTIFICATION {person_id: p1.id}]-(p2:Person) return {person: p1, certification: collect({name: c1.name, certification_id: c1.id,  expired_at: r1.expired_at, signed_by_first_name: p2.first_name, signed_by_last_name: p2.last_name, signed_by_id: p2.id})}"  
  neode.cypher(query, {})
  .then((collection) => {
    const data = collection.records.map((item) => {
      return {
